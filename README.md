...
**HackForge** is a full-stack application that combines **Next.js** on the frontend with a **Node.js + Express backend**, powered by **Groq** (or xAI) to intelligently generate complete, deployable codebases from plain English prompts.  
...
| **Backend**    | Node.js, Express.js, MongoDB, Mongoose, Groq/xAI, JWT, Redis, CORS |
...
- A Groq API key ([console.groq.com](https://console.groq.com/keys)) or an xAI key  
...
```bash
PORT=5002
# Groq keys start with gsk_, xAI keys with xai- — the provider is detected
# from the prefix, so no extra configuration is needed.
GROQ_API_KEY=your_groq_api_key

# Optional. Defaults to llama-3.3-70b-versatile on Groq.
...
Verify the backend is wired up correctly — `services.ai` must be `true`
