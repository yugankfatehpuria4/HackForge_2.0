...
export const blogPosts: BlogPost[] = [
  {
    ...
    body: [
      ...
      'The request is sent to Gemini with a deliberately moderate temperature. Higher values produce more inventive code and considerably more hallucinated APIs. Around 0.7 the model stays creative on structure while remaining conservative about the libraries it reaches for.',
      'Output is capped at a token budget large enough for multi-file scaffolds. When a response is truncated the backend detects the MAX_TOKENS finish reason and surfaces a warning rather than handing back a file that stops halfway through a function.',
    ...
    steps: [
      {
        ...
        detail:
          'Run npm run backend:dev and npm run dev in separate terminals, then open http://localhost:3000. Check http://localhost:5002/health first — services.gemini must be true.',
      },
      {
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        detail:
          'The Gemini key belongs to the backend service only. Anything prefixed NEXT_PUBLIC_ is compiled into the browser bundle and is not a secret.',
      },
    ],
  },
];
